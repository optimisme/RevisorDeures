# Prompt: Generar la llista de tasques atòmiques

Aquest prompt genera el fitxer `tasks/tasks.md` amb totes les tasques atòmiques necessàries per desenvolupar el servidor web de RevisorDeures.

Segueix les instruccions pas a pas:

## Objectiu

Crear `tasks/tasks.md` amb una llista de tasques definides mitjançant **descomposició atòmica**. Cada tasca ha de ser una unitat de treball independent que:

1. Tingui un abast clar i limitat
2. Tingui els tests associats definits
3. Tingui un estat que es pugui seguir fàcilment: `pendent`, `implementant`, `revisant`, `completada`
4. Estigui agrupada per fases (segons el PLAN.md)

## Estructura del fitxer tasks/tasks.md

El fitxer ha de tenir el següent format per a cada tasca:

```markdown
### T{fase}.{número} — {títol de la tasca}

**Estat:** [PENDENT]
**Prioritat:** alta/médium/baixa
**Agent assignat:** {nom de l'agent}
**Dependències:** T{X}.{Y}, ...

{descripció detallada de què cal fer}
...
Les regles al capdamunt del fitxer:
...
- Agent: coordinator
- Dependències: cap
- Descripció: Crear la estructura completa de directoris del projecte:
...
- Tests:
  - [ ] Executar `ls -R server/` i verificar que existeixen totes les carpetes

**T0.2 — Crear package.json amb dependències**
- Agent: backend-dev
- Dependències: T0.1
- Descripció: Crear `package.json` amb les dependències: express, better-sqlite3, bcrypt, dotenv, cors, express-session.
- Tests:
  - [ ] Executar `npm install` sense errors
  - [ ] Verificar que totes les dependències es poden requerir des de Node.js

**T0.3 — Crear server/settings.env**
- Agent: coordinator
- Dependències: cap
- Descripció: Crear `server/settings.env` amb SERVER_ADMIN_PWD, DATABASE_PATH, PORT=3000, SESSION_SECRET (generrat aleatòriament), OPENCODE_ZEN_API_KEY, PROXY_AGENTS_BASE_URL, PROXY_AGENTS_KEY, GITHUB_PERSONAL_ACCESS_TOKEN (tot agafat de variables d'entorn del projecte).
- Tests:
  - [ ] Verificar que `server/settings.env` existeix
  - [ ] Verificar que té totes les variables necessàries

**T0.4 — Crear servidor bàsic Express**
- Agent: backend-dev
- Dependències: T0.2, T0.3
- Descripció: Crear `server/index.js` amb: dotenv, Express, CORS, express-session, servei d'estàtics des de server/public/, rutes API a /api/*.js, llistat al PORT, middleware JSON, error handler.
- Tests:
  - [ ] Executar `node server/index.js` i verificar que es connecta al port
  - [ ] Fer `curl http://localhost:3000/` i verificar que respon
  - [ ] Fer `curl http://localhost:3000/api` i verificar que respon

**T0.5 — Crear inicialització de base de dades**
- Agent: backend-dev
- Dependències: T0.4
- Descripció: Crear `server/db/init.js` que connecta a SQLite, verifica connexió, fitxer server/data/data.db es crea automàticament.
- Tests:
  - [ ] Executar `node server/db/init.js` sense errors
  - [ ] Verificar que es crea `server/data/data.db`
...
- Agent: backend-dev
- Dependències: T0.5
- Descripció: Definir a `server/db/init.js` les taules:
...
- Tests:
  - [ ] Executar `node server/db/init.js` sense errors
  - [ ] Obrir server/data/data.db amb sqlite3 i verificar .schema

**T1.2 — Capa de dades: alumnes**
- Agent: backend-dev
- Dependències: T1.1
- Descripció: Crear `server/db/alumnes.js` amb: getAll(), getById(), getByEmail(), create({nom, email, password}), update(id, {nom, email}), delete(id), existsByEmail(email). Hash MD5 amb crypto.createHash('md5').
- Tests:
  - [ ] Crear un alumne amb email vàlid i verificar que es guarda
  - [ ] Crear amb email duplicat i verificar error
  - [ ] Consultar per email i verificar que es recupera
  - [ ] Verificar que el hash MD5 es guarda (no text pla)

**T1.3 — Capa de dades: practiques**
- Agent: backend-dev
- Dependències: T1.1
- Descripció: Crear `server/db/practiques.js` amb: getAll(), getById(), create({titol, criteria}), update(id, {titol, criteria}), delete(id), exists(titol).
- Tests:
  - [ ] Crear una pràctica i verificar
  - [ ] Consultar totes
  - [ ] Actualitzar
  - [ ] Esborrar
...
- Agent: backend-dev
- Dependències: T1.1
- Descripció: Crear `server/db/entregues.js` amb: getAll(), getById(), getByAlumne(alumne_id), getByPractica(practica_id), getNoRevisadesByAlumne(alumne_id), create({alumne_id, practica_id, repo_url}), updateEstat(id, estat), marcarRevisada(id, revisat_per), delete(id).
- Tests:
  - [ ] Crear entrega
  - [ ] Consultar per alumne
  - [ ] Consultar per practica
  - [ ] Marcar com revisada
  - [ ] Consultar no revisades d'un alumne

**T1.5 — Capa de dades: valoracions**
- Agent: backend-dev
- Dependències: T1.1
- Descripció: Crear `server/db/valoracions.js` amb: getByEntrega(entrega_id), create({entrega_id, ...}), updateEstat(id, estat), updateResultat(id, resultat, comentaris, detall).
- Tests:
  - [ ] Crear valoració per entrega
  - [ ] Consultar per entrega
  - [ ] Actualitzar estat
...
- Descripció: Crear `server/routes/auth.js`:
...
- Tests:
  - [ ] Login admin correcte → 200 rol admin
  - [ ] Login admin incorrecte → 401
  - [ ] Login alumne correcte → 200 rol alumne
  - [ ] Login alumne incorrecte → 401
  - [ ] GET /api/auth/session amb sessió → dades
  - [ ] GET /api/auth/session sense sessió → null
  - [ ] POST /api/auth/logout → sessió destruïda

**T2.2 — Pàgina de login**
- Agent: frontend-dev
- Dependències: T2.1
- Descripció: Crear `server/public/index.html` amb formulari de login (usuari, password), JavaScript client que fa POST a /api/auth/login, redirigeix a /admin si admin, /alumne si alumne, mostra errors.
- També crear `server/public/admin.html` (esquelet) i `server/public/alumne.html` (esquelet).
- Tests (Playwright):
  - [ ] Obrir / → formulari de login visible
  - [ ] Login admin correcte → redirigeix a /admin
  - [ ] Credencials incorrectes → mostra error
  - [ ] Formulari enviat amb Enter funciona
  - [ ] admin.html accessible després de login
  - [ ] alumne.html accessible després de login
...
- Descripció: Rutes protegides amb requireAuth('admin') a server/routes/index.js:
...
- Tests:
  - [ ] GET sense sessió → 401
  - [ ] GET amb admin → 200 array buit
  - [ ] POST correcte → 201
  - [ ] POST email duplicat → 400
  - [ ] POST camps buits → 400
  - [ ] GET /alumnes/:id → 200 sense password
  - [ ] PUT /alumnes/:id → 200 actualitzat
  - [ ] DELETE /alumnes/:id → 200
  - [ ] DELETE 999 → 404
  - [ ] POST amb sessió alumne → 403

**T3.2 — Pàgina de gestió d'alumnes**
- Agent: frontend-dev
- Dependències: T3.1
- Descripció: Crear `server/public/admin-alumnes.html`:
...
- Tests (Playwright):
  - [ ] Obrir /admin/alumnes sense sessió → redirigeix a /
  - [ ] Obrir /admin/alumnes amb admin → mostra pàgina
  - [ ] Mostrar llista d'alumnes
  - [ ] Afegir alumne → actualitza taula
  - [ ] Esborrar alumne → confirma i actualitza
  - [ ] Botó "Entregues" → /admin/entregues?alumne=X
  - [ ] Navegació "Tornar a l'administrador" funciona
  - [ ] Alumne que accedeix a /admin/alumnes → redirigeix a /alumne
...
- Descripció: Rutes protegides a server/routes/index.js:
  - GET /api/practiques
...
- Tests:
  ...
  - [ ] POST correcte → 201
  - [ ] POST títol buit → 400
  - [ ] PUT /practiques/:id → 200
  - [ ] DELETE /practiques/:id → 200
  - [ ] DELETE 999 → 404
  - [ ] POST amb alumne → 403
...
- Agent: frontend-dev
- Dependències: T4.1
- Descripció: Crear `server/public/admin-practiques.html`:
  ...
  - Navegació a /admin
- Tests (Playwright):
  - [ ] Obrir /admin/practiques sense sessió → redirigeix a /
  - [ ] Amb sessió admin → mostra pàgina
  - [ ] Mostrar llista
  - [ ] Afegir pràctica amb formulari
  - [ ] Editar pràctica
  - [ ] Esborrar amb confirmació
  - [ ] Botó "Entregues" → /admin/entregues?practica=X
...
- Descripció: Ruta a server/routes/index.js:
...
- Tests:
  - [ ] POST sense sessió → 401
  - [ ] POST amb admin → 403
  - [ ] POST amb alumne i dades correctes → 201
  - [ ] POST URL no GitHub → 400
  - [ ] POST practica inexistent → 400

**T5.2 — Consulta i eliminació d'entregues**
- Agent: backend-dev
- Dependències: T1.4, T5.1
- Descripció: Rutes a server/routes/index.js:
  ...
  - DELETE /api/entregues/:id → alumne només si revisada=false (soft delete a 'esborrada'); admin pot esborrar qualsevol
- Tests:
  - [ ] GET sense sessió → 401
  - [ ] GET amb alumne → les seves
  - [ ] GET amb admin → totes
  - [ ] GET ?practica_id=X → filtra
  - [ ] GET /alumne/:id amb admin → 200
  - [ ] GET /practica/:id amb admin → 200
  - [ ] DELETE alumne no revisada → 200
  - [ ] DELETE alumne revisada → 403
  - [ ] DELETE admin → 200
...
- Descripció: Rutes a server/routes/index.js:
  ...
  - GET /api/valoracions/entrega/:entrega_id → retorna valoració; alumne només la seva, admin qualsevol
- Tests:
  - [ ] PATCH amb admin → 200, revisada=true
  - [ ] PATCH si ja revisada → 400
  - [ ] GET /pendents amb admin → 200
  - [ ] PATCH amb alumne → 403
  - [ ] GET valoració amb admin → 200
  - [ ] GET valoració alumne no propietari → 403

**T5.4 — Pàgines d'entregues de l'alumne**
- Agent: frontend-dev
- Dependències: T5.1, T5.2, T5.3
- Descripció: Crear `server/public/alumne.html` (espai principal alumne) i `server/public/alumne-entregues.html` i `server/public/alumne-enviar.html`:
...
- Agent: frontend-dev
- Dependències: T5.3
- Descripció: Crear `server/public/admin-entregues.html`:
...
- Agent: ai-reviewer
- Dependències: T1.5
- Descripció: Crear `server/evaluator.js`:
...
- Tests:
  - [ ] La funció valorarEntrega es pot cridar (test mock)
...
- Descripció: Completar `server/public/admin.html` amb:
...
- Tests (Playwright):
  - [ ] Obrir /admin amb sessió → mostra targetes
  - [ ] Clic a cada targeta → redirigeix correctament
  - [ ] Botó logout → destrueix sessió, redirigeix a /
...
- Tests (Playwright):
  - [ ] Totes les pàgines carreguen amb els estils correctes
  - [ ] Els formularis es mostren correctament
  - [ ] Les taules es mostren correctament
  - [ ] El disseny responsive funciona (provar amb diferents mides)
...
- Tests (Playwright):
  - [ ] Flux login admin → CRUD alumnes → logout
  - [ ] Flux login alumne → enviar entrega → veure entregues → logout
  - [ ] Flux admin revisar entrega → alumne veu estat
  - [ ] Flux completar valoració automàtica → alumne veu resultat
  - [ ] Totes les rutes API responen correctament
